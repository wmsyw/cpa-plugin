...
func (r *modelRuntime) ensureForAuth(req authModelRequestWire) modelReadinessSnapshot {
	...
	if err != nil {
		...
		logrus.WithError(err).Warn("wbglobal-auth-diag")
	...
	snapshot.Models = models
	logrus.WithFields(logrus.Fields{"state": snapshot.State, "ec": snapshot.ErrorCode, "ms": snapshot.ModelSource, "mds": snapshot.MetadataSource}).Warn("wbglobal-final-diag")
