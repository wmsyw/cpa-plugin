...
func deleteAuth(authIndex, authID string, sa *storedAuth) error {
	...
	// Also remove legacy wbglobal.json if this UID was dual-named historically.
...
func resolveAuthIndexAndID(authID string) (string, string) {
	...
	// Multi-account files are wbglobal-<uid>.json; list Name/ID usually carry that.
	wantName := "wbglobal-" + authID + ".json"
	for _, f := range files {
...
func invalidateAccountCredits(authID, authUID string) {
	...
	wantName := "wbglobal-" + authUID + ".json"
	matchedByName := false
	for _, f := range files {
		if f.AuthIndex == authID || f.ID == authID || f.Name == authID {
...
// (wbglobal-<uid>.json naming). Pure helper for O(list) cache invalidation.
func listEntryMatchesUID(f pluginapi.HostAuthFileEntry, uid, wantName string) bool {
	...
	return strings.EqualFold(base, "wbglobal-"+uid)
}
