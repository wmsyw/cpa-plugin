...
func TestModelAuthIdentityUsesTrimmedAuthIDWhenUIDMissing(t *testing.T) {
	...
	// Opaque token: the realm falls back to accountRegion, which is
	// constant-global in this plugin.
	if identity.Provider != "workbuddy-global" || identity.Realm != workBuddyRealmGlobal || identity.UID != "" || identity.EnterpriseID != "enterprise-1" || identity.AuthID != "auth-index-1" {
...
func TestModelAuthIdentityHashChangesWithRealmAndEnterprise(t *testing.T) {
	...
	otherEnterprise := syntheticStoredAuth(t, workBuddyRealmGlobal)
...
func TestModelAuthIdentityHashIsLowercaseSHA256Filename(t *testing.T) {
	identity, err := modelAuthIdentityFor("auth-1", syntheticStoredAuth(t, workBuddyRealmGlobal))
	if err != nil {
		t.Fatal(err)
	}
...
func TestModelStoreSchemaV1RoundTripsAtExactPaths(t *testing.T) {
	...
	loadedModels, found, err := store.loadModels(hash, workBuddyRealmGlobal)
...
func TestModelStoreIdentityMismatchReturnsNoValidCache(t *testing.T) {
	...
	got, found, err := newModelStore(root).loadModels(requestedHash, workBuddyRealmGlobal)
	if err == nil || found {
