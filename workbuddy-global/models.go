...
func wbModels() []pluginapi.ModelInfo {
	mk := func(id, name string, ctxLen, maxTok int64) pluginapi.ModelInfo {
		return pluginapi.ModelInfo{ID: id, Name: name, DisplayName: name, ContextLength: ctxLen, MaxCompletionTokens: maxTok, OwnedBy: providerName, SupportedGenerationMethods: []string{"chat"}}
	}
	return []pluginapi.ModelInfo{
		mk("hy4-preview", "Hy4 preview", 1000000, 64000),
		mk("hy3", "Hy3", 192000, 64000),
		mk("gpt-5.6-sol", "GPT-5.6-Sol", 372000, 128000),
		mk("gpt-5.6-terra", "GPT-5.6-Terra", 372000, 128000),
		mk("gpt-5.6-luna", "GPT-5.6-Luna", 372000, 128000),
		mk("gpt-5.5", "GPT-5.5", 272000, 128000),
		mk("gpt-5.4", "GPT-5.4", 1050000, 128000),
		mk("gpt-5.3-codex", "GPT-5.3-Codex", 128000, 128000),
		mk("gemini-3.5-flash", "Gemini-3.5-Flash", 1048576, 65536),
		mk("glm-5.3", "GLM-5.3", 1000000, 131072),
		mk("glm-5.2", "GLM-5.2", 1000000, 131072),
		mk("kimi-k3", "Kimi-K3", 1000000, 131072),
		mk("kimi-k2.6", "Kimi-K2.6", 262144, 131072),
		mk("minimax-m3", "MiniMax-M3", 204800, 131072),
	}
}

// verifiedChatFallbackIDs covers the complete Global CLI catalog currently
// confirmed on the chat route. The personal models endpoint currently fails
// with HTTP 500, so retain this exact catalog when dynamic discovery fails.
...
func officialMaxCompletionTokens(modelID string, fallback int64) int64 {
	switch modelID {
	case "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna", "gpt-5.5", "gpt-5.4", "gpt-5.3-codex":
		return 128000
	case "gemini-3.5-flash":
		return 65536
	case "glm-5.3", "glm-5.2", "kimi-k3", "kimi-k2.6", "minimax-m3":
		return 131072
	case "hy3", "hy4-preview":
		return 64000
	}
	return fallback
}
...
// name across different model IDs.
...
// isGlobalToken decodes the JWT iss claim and accepts only an exact Global
// domain or one of its subdomains. Substring checks would accept attacker-owned
// values such as evilworkbuddy.ai.
func isGlobalToken(accessToken string) bool {
	...
	if parsed, err := url.Parse(iss); err == nil && parsed.Hostname() != "" {
		return isGlobalDomain(parsed.Hostname())
	}
	iss = strings.ToLower(iss)
	iss = strings.TrimPrefix(strings.TrimPrefix(iss, "https://"), "http://")
	if slash := strings.IndexByte(iss, '/'); slash >= 0 {
		iss = iss[:slash]
	}
	return isGlobalDomain(iss)
}
...
func callModelsAPI(accessToken string) ([]pluginapi.ModelInfo, error) {
	...
	defer cancel()
	// This provider is Global-only; never derive a CN fallback from token shape.
