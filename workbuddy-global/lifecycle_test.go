...
func TestDisplayNote(t *testing.T) {
	...
	note := displayNote(gl, &creditsSummary{TotalRemain: 12, TotalUsed: 8, TotalSize: 20}, false)
	if !strings.Contains(note, "Global") || !strings.Contains(note, "12") || !strings.Contains(note, "8") {
	...
	if !strings.Contains(note, "Global") || !strings.Contains(note, "耗尽") {
		t.Fatalf("global exhausted note = %q", note)
	}
}

func TestBuildAuthFileJSON_ContainsDisabledAndNote(t *testing.T) {
	sa := &storedAuth{
		Auth:    storedTokens{AccessToken: "at", RefreshToken: "rt", Domain: "www.workbuddy.ai"},
		Account: storedAccount{UID: "u1", Nickname: "nick"},
	}
	raw, err := buildAuthFileJSON(sa, true, "Global · test", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSafeWorkbuddyAuthPath(t *testing.T) {
	dir := t.TempDir()
	ok := filepath.Join(dir, "wbglobal-abc.json")
...
func TestDeleteAuthFile_Idempotent(t *testing.T) {
	dir := t.TempDir()
	p := filepath.Join(dir, "wbglobal-x.json")
...
func TestAuthFileNameFor(t *testing.T) {
	...
	if got := authFileNameFor(sa); got != "wbglobal-uid-1.json" {
		t.Fatalf("got %q", got)
	}
...
func TestIsSafeWorkbuddyAuthPath(t *testing.T) {
	dir := t.TempDir()
	ok := filepath.Join(dir, "wbglobal-safe.json")
	if !isSafeWorkbuddyAuthPath(ok) {
		t.Fatalf("want safe: %s", ok)
	}
	legacy := filepath.Join(dir, "wbglobal.json")
	...
	if isSafeWorkbuddyAuthPath("/auth/../wbglobal-x.json") {
		t.Fatal("path with .. must be rejected")
	}
	// /etc/wbglobal-evil.json has valid basename but is NOT under auth dir.
...
func TestDeleteAuthFileInDir(t *testing.T) {
	dir := t.TempDir()
	target := filepath.Join(dir, "wbglobal-x.json")
	...
	if err := deleteAuthFileInDir("wbglobal-evil.json", ""); err == nil {
...
func TestListEntryMatchesUID(t *testing.T) {
	...
	cases := []struct {
		...
		{"basename id", pluginapi.HostAuthFileEntry{Name: "wbglobal-" + uid}, true},
		{"case", pluginapi.HostAuthFileEntry{Name: strings.ToUpper(want)}, true},
		{"other uid", pluginapi.HostAuthFileEntry{Name: "wbglobal-other.json"}, false},
		{"legacy bare", pluginapi.HostAuthFileEntry{Name: "wbglobal.json"}, false},
