	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got := runtime.ensureForAuth(authModelRequestWire{
				AuthModelRequest: pluginapi.AuthModelRequest{AuthID: "auth-configured-matrix", StorageJSON: mustJSON(syntheticStoredAuth(t, workBuddyRealmGlobal))},
...
func TestConfiguredModelsValidateAuthIdentityAndTokenGeneration(t *testing.T) {
	...
	unsupportedRealm := syntheticStoredAuth(t, workBuddyRealmGlobal)
	unsupportedRealm.Auth.AccessToken = syntheticAccessToken(t, "https://unsupported.example/realms/cli")
	missingIdentity := syntheticStoredAuth(t, workBuddyRealmGlobal)
	...
	wwwIssuerAuth := syntheticStoredAuth(t, workBuddyRealmGlobal)
	wwwIssuerAuth.Auth.AccessToken = syntheticAccessToken(t, "https://www.codebuddy.ai/auth/realms/copilot")
	wwwIssuerAuth.Auth.Domain = "codebuddy.ai"
	...
	secondAuth.Auth.AccessToken = syntheticAccessToken(t, "https://codebuddy.ai/realms/other")
