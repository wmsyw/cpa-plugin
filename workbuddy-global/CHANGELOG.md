...
### Global model catalog correction

- `models.go` — align the static and per-account fallback catalog with the
  currently usable Global models: Hy4 preview, Hy3, GPT-5.6 Sol/Terra/Luna,
  GPT-5.5, GPT-5.4, GPT-5.3-Codex, Gemini-3.5-Flash, GLM-5.3, GLM-5.2,
  Kimi-K3, Kimi-K2.6, and MiniMax-M3.
- Use Global's actual `kimi-k3` upstream ID; `kimi-k3-1` is rejected by the
  Global chat endpoint.
- `payload.go` — map `max` to `xhigh` for GPT-5.5/5.4/Codex, map Gemini's
  `none`/`xhigh` to `minimal`/`high`, and keep the full native ladder for
  GPT-5.6, Kimi, and MiniMax routes.

## 0.9.1-global.1

### 可用模型 + 思考强度映射

- `models.go` — 静态兜底清单补入 `kimi-k2.6`（Kimi-K2.6，上下文 262144 /
  输出 131072）。上游 CLI agent 目录已发布该模型，`officialMaxCompletionTokens`
  也早已为它预留 131072，但 `wbModels()` 一直没有它；Global 的
  `/console/enterprises/personal/models` 目前恒返回 500（APISIX 层，两个账号
  均复现），`fetchDynamicModelsFromStorage` 只能落到静态清单，因此该模型在
  Global 侧此前永远不可见。
- 思考强度映射不需要为 `kimi-k2.6` 加分支：上游 reasoning 形状为
  `{"effort":"medium"}`（与 `kimi-k2.7` 相同），实测 none/low/medium/high/
  xhigh/max 六档全部返回 200，且 `max` 产出最深 reasoning，说明原生支持完整
  阶梯，`mapReasoningEffortInPlace` 直通即为正确行为。
- `sanitize_test.go` / `models_public_test.go` — 新增用例锁定该契约：
  `kimi-k2.6` 六档 effort 全部透传不改写、输出上限 131072、出现在公开清单中。
...
- **UID 路径穿越修复**：`authFileNameFor` 新增 `sanitizeUIDForFileName` 白名单
  ...
  `wbglobal-<uid>.json` 不再可能被 `../` 注入到任意路径。
