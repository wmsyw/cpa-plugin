...
func handleImportAuth(req pluginapi.ManagementRequest) map[string]any {
	...
	// Remove legacy wbglobal.json if it exists and differs from the saved name.
	if saveResp.Name != "" && !strings.EqualFold(saveResp.Name, authFileName) {
		legacyPath := strings.TrimSpace(saveResp.Path)
		// Best-effort: if auth dir is known via saveResp.Path parent, try removing sibling wbglobal.json.
		if legacyPath != "" {
