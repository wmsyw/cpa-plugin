...
// wbglobal-<uid>.json naming rule, UID sanitization (path-traversal defense),
...
// authFileNameFor matches toAuthData naming: always wbglobal-<uid>.json when UID is known.
// Bare "wbglobal.json" is legacy single-account only (no UID).
...
// with multi-account wbglobal-<uid>.json for the same credential.
...
// Prefer wbglobal-<uid>.json; if the host still points at legacy wbglobal.json
...
// when the canonical name differs (wbglobal.json → wbglobal-<uid>.json).
...
func isSafeWorkbuddyAuthPath(path string) bool {
	...
	if !strings.HasPrefix(lower, "wbglobal-") && lower != "wbglobal.json" {
		return false
	}
