...
func TestModelRuntimeFreshBootstrapReady(t *testing.T) {
	...
	do := func(req *http.Request, callbackID string) (*hostHTTPResponse, error) {
		...
		switch {
		case req.URL.Host == "www.workbuddy.ai" && req.URL.Path == "/v3/config":
	...
	if _, found, err := store.loadModels(identity.sha256(), workBuddyRealmGlobal); err != nil || !found {
...
func TestModelRuntimeSnapshotImmutable(t *testing.T) {
	...
	published := runtime.ensureForAuth(authModelRequestWire{
		AuthModelRequest: pluginapi.AuthModelRequest{AuthID: "auth-copy", StorageJSON: mustJSON(syntheticStoredAuth(t, workBuddyRealmGlobal))},
...
func TestModelRuntimeSameAuthSingleflight(t *testing.T) {
	...
	runtime := newModelRuntime(newModelStore(t.TempDir()), do)
	req := authModelRequestWire{AuthModelRequest: pluginapi.AuthModelRequest{AuthID: "auth-one", StorageJSON: mustJSON(syntheticStoredAuth(t, workBuddyRealmGlobal))}}
...
func TestModelRuntimeDifferentAuthIsolation(t *testing.T) {
	...
	do := func(req *http.Request, callbackID string) (*hostHTTPResponse, error) {
		// Both auths are Global-realm (the decoder rejects CN issuers); the
		// per-auth isolation is proven by distinct token signatures.
		switch {
		case strings.HasSuffix(req.Header.Get("Authorization"), "signature-cn"):
			if cnCalls.Add(1) == 1 {
				close(cnStarted)
			}
			<-cnRelease
		...
		case strings.HasSuffix(req.Header.Get("Authorization"), "signature"):
	...
	runtime := newModelRuntime(newModelStore(t.TempDir()), do)
	cnAuth := syntheticStoredAuth(t, workBuddyRealmGlobal)
	cnParts := strings.Split(cnAuth.Auth.AccessToken, ".")
	cnAuth.Auth.AccessToken = cnParts[0] + "." + cnParts[1] + ".signature-cn"
...
func TestModelRuntimeMetadataSingleflight(t *testing.T) {
	...
	runtime := newModelRuntime(newModelStore(t.TempDir()), do)
	firstAuth := syntheticStoredAuth(t, workBuddyRealmGlobal)
	firstAuth.Account.UID = "uid-one"
	firstAuth.Account.EnterpriseID = "enterprise-one"
	secondAuth := syntheticStoredAuth(t, workBuddyRealmGlobal)
...
func TestModelRuntimeConcurrentReaders(t *testing.T) {
	...
	go func() {
		bootstrapDone <- runtime.ensureForAuth(authModelRequestWire{
			AuthModelRequest: pluginapi.AuthModelRequest{AuthID: "auth-readers", StorageJSON: mustJSON(syntheticStoredAuth(t, workBuddyRealmGlobal))},
		})
	}()
	<-started
...
func TestModelRuntimeOldGenerationCannotCommit(t *testing.T) {
	...
	store := newModelStore(root)
	runtime := newModelRuntime(store, do)
	saOld := syntheticStoredAuth(t, workBuddyRealmGlobal)
	...
	cached, found, err := store.loadModels(identity.sha256(), workBuddyRealmGlobal)
...
func TestModelRuntimeSharedIdentityFailureDoesNotDiscardConcurrentSuccess(t *testing.T) {
	...
	store := newModelStore(root)
	runtime := newModelRuntime(store, do)
	saGood := syntheticStoredAuth(t, workBuddyRealmGlobal)
...
func TestModelRuntimeFailedConfigureKeepsGeneration(t *testing.T) {
	...
	failures := []struct {
		...
		{name: "feature parse", raw: mustJSON(map[string]any{"config_yaml": []byte("models: [serve-alpha, ' serve-alpha ']\n")})},
	}
