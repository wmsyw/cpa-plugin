...
- **OAuth 登录** — 通过宿主 auth store 管理多账号 `wbglobal-<uid>.json`，
  本副本仅处理国际版；国内版由原 workbuddy 插件独立处理。
...
- **执行器** — OpenAI 兼容 chat completions，流式（真 SSE，走 `host.stream.emit`）
  ...
  Claude Code 模板清洗、按国际版规则注入 system message。
...
- **调度器**（可选） — `off`（默认）完全交给 CPA 内置调度；`credits`
  保持使用面板选中的粘性账号；`expiry` 根据缓存套餐的 `DeductionEndTime`，
  选择当前可消费积分最早过期的账号。`expiry` 路由绝不会等待 billing API，
  缓存过期或未知的账号仍作为回退候选。
...
Global 静态兜底清单与当前客户端可用模型保持一致：

`hy4-preview`、`hy3`、`gpt-5.6-sol`、`gpt-5.6-terra`、`gpt-5.6-luna`、
`gpt-5.5`、`gpt-5.4`、`gpt-5.3-codex`、`gemini-3.5-flash`、`glm-5.3`、
`glm-5.2`、`kimi-k3`、`kimi-k2.6`、`minimax-m3`。

上游个人模型目录接口异常时，插件使用该清单；Global 聊天接口实际接受的
Kimi K3 ID 是 `kimi-k3`，不是旧目录曾出现的 `kimi-k3-1`。
...
每个账号登录一次，插件会把 `wbglobal-<uid>.json` 写入 auth store。

### 4. 调用

用任何映射到 WorkBuddy Global 模型的 alias 调 OpenAI 兼容端点：
...
plugins:
  configs:
    workbuddy-global:
      ...
      # 调度行为（默认 "off"）：
      #   off     → 完全交给 CPA 内置调度
      #   credits → 保持使用面板选中的粘性账号
      #   expiry  → 根据缓存套餐 DeductionEndTime，选择当前可消费积分
      #             最早过期的账号；缓存过期/未知的账号作为回退候选，
      #             路由绝不会等待 billing API
