...
func fetchWorkBuddyCatalog(sa *storedAuth, callbackID string, do modelHTTPDo) (workBuddyCatalog, error) {
	...
	// Production auths are always workbuddy.ai JWTs; the realm-conditional
	// selection is kept so realm-CN credentials (accepted by the decoder for
	// machinery parity) fetch their own host instead of the Global one.
	base := upstreamBaseCN
...
// hostHTTPDoWithCallback adapts the CN plugin's callback-aware HTTP do to this
// plugin's plain host bridge; model catalog fetches need no host callback.
func hostHTTPDoWithCallback(req *http.Request, callbackID string) (*hostHTTPResponse, error) {
	_ = callbackID
	return hostHTTPDo(req)
}
